...
const JsonViewer = ({ data, rootKey = 'root' }) => {
...
const JsonNode = ({ data, keyName, level = 0, isLast = true, searchTerm = '' }) => {
  ...
  const indentClass = `json-indent-${level}`;

  return (
    <div className={`json-property ${indentClass}`}>
      {keyName && (
...
const ObjectNode = ({ data, keyName, level = 0, searchTerm = '' }) => {
...
const ArrayNode = ({ data, keyName, level = 0, searchTerm = '' }) => {
  const [isCollapsed, setIsCollapsed] = useState(level > 1);
